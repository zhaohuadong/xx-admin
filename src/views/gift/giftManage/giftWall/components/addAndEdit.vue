  ...
  <div>
    <MyDialog :model-value="visible" :title="titleName" @submit="submit" @toggle="toggle">
      <el-form ref="formRef" :model="form" :rules="formRule" label-width="auto">
        <el-form-item label="礼物墙类别名称" prop="wallName">
          <el-input v-model="form.wallName" placeholder="请输入礼物墙类别名称" />
        </el-form-item>
      </el-form>
    </MyDialog>
  </div>
...
<script setup>
import { editApi, addApi } from '@/api/gift/giftWall.js'
...
const showDialog = (params) => {
  ...
  titleName.value = params !== undefined ? '编辑礼物墙类别' : '新增礼物墙类别'
