  ...
  <MyDialog :model-value="visible" :title="titleName" @submit="submit" @toggle="toggle">
    <el-form ref="formRef" :model="form" :rules="formRule" label-width="auto">
      <el-form-item label="用户编号" prop="userCode">
        <el-input v-model.number="form.userCode" placeholder="请输入用户编号" clearable class="w-full" />
      </el-form-item>
      <el-form-item label="绑定奖池" prop="types">
        <el-checkbox-group v-model="form.types">
          <el-checkbox v-for="(item, index) in poolList" :key="index" :label="item.id">
            {{ item.name }}
          </el-checkbox>
        </el-checkbox-group>
      </el-form-item>
...
<script setup>
import { getSpecialListApi } from '@/api/game/specialPoolConfiguration.js'
import { editApi, addApi } from '@/api/game/specialPoolUserList.js'
