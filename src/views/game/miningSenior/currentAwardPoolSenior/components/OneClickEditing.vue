...
import { oneEdit, getListApi } from '@/api/game/nextCurrentAwardPool.js'
...
const showDialog = (params) => {
  ...
  if (props.nextConfigId) {
    ;(async function () {
      const { rows } = await getListApi({ type: params, nextConfigId: props.nextConfigId })
      editGiftList.value = rows
    })()
  } else {
    // 获取礼物数据
    ;(async function () {
      const { rows } = await getListApi({ type: params })
      editGiftList.value = rows
    })()
  }
