...
<script setup name="BlindBoxNextPool">
import { columns, blindBoxList } from './constants.js'
import { getListApi, deleteDownApi, batchDownDeleteApi, getTotalListApi, batchEditApi } from '@/api/game/blindbox.js'
...
const gitStatList = async (params) => {
  const { data } = await getTotalListApi({ type: params, nextJackpot: 1 })
...
const deleteList = (params) => {
  ...
  return deleteDownApi(newParams)
}

// 批量删除
const deleteBatchList = (params) => {
  const newParams = JSON.parse(JSON.stringify(params))
  return batchDownDeleteApi(newParams, initParam.type)
}
</script>
